...
export class UIAdapter{
    ...
    public static getUILayout(myControls: Array<BaseControl>, repDLG: rep8.ReportDialog): any{
        let UIControl: UIStyleControl = new UIStyleControl();
        let mySection: BaseControl = new BaseControl();
        mySection.type = 'mySection';
        let title: string = repDLG.m_MYDLGTEMPLATE.caption;
        let mID = title;
        while(1){
            if(mID.indexOf(' ')!=-1){
        ...
        mySection.options.id = mID;
        mySection.options.klass = 'section-border';

        let pageTitle: BaseControl = new BaseControl();
        pageTitle.type = 'myTitle';
        pageTitle.options.title = title;        
        pageTitle.options.level = 3;

        if(mySection.options.children === undefined){
            mySection.options.children = [];
        }
        mySection.options.children.push(pageTitle);
        ...
        for(let i: number = 0; i < bodycontrol.length; i++){
            ...
            if(bodycontrol[i].options.exData === '$_asdtxt1'){
                ...
                mySection.options.children.push(bodycontrol[i]);
                continue;
            }
            if(mySection.options.children === undefined){
                mySection.options.children = [];
        ...
        let mySpan: BaseControl = new BaseControl();
        mySpan.type = 'mySpan';
        mySpan.options.klass = "";
        mySpan.options.children = [];
        for(let i: number = 0; i < footcontrol.length; i++){
            mySpan.options.children.push(footcontrol[i]);
        }
        mySection.options.children.push(mySpan);
        UIControl.children.push(mySection);
        
        
        console.warn(UIControl);

        return UIControl;
    }

    private static FixforUI(repDLG: rep8.ReportDialog){
        console.warn(repDLG);
        let mytempControls: Array<BaseControl> = [];
        let myControls: Array<BaseControl> = [];
        let dlgarray: Array<rep8.MYDLGITEMTEMPLATE> = repDLG.m_DLGITEMTEMPLATE;
